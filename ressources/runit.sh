...
# argv[0]="./a.out" TODO
if [[ "$OSTYPE" == "msys" ]]; then
    winpty docker run -it --log-driver none --privileged -e "DIR=$1" -e "ARGC=$#" -e "ARGV=$argv" inf2610-code-runner-image /bin/bash
else
    docker run -it --log-driver none --privileged -e "DIR=$1" -e "ARGC=$#" -e "ARGV=$argv" inf2610-code-runner-image /bin/bash
fi